...
class LoginSeonmi extends Component {
    ...
    handleButton = () => {
        const {isBtnActive} = this.state; // checkBtnState가 있는데 필요한가?
        const {IdValue, PwValue} = this.state;
        if (!isBtnActive) {
            alert("탈락!")
            return;
        ...
        fetch(SIGNOUT_API, {
            ...
            .then((result) => {
                ...
                this.setState({ token: result.Authorization}); // 지우기!!!

                // 3-3) response 메세지에 따른 로직 처리
                if(result.message === "SUCCESS") {
                    localStorage.setItem('token', result.Authorization);      // 항상 Authorization 아님
                    alert("로그인에 성공하셨습니다.");
                ...
                <div className="Login">
                    <div className="loginBoxDiv">
                        ...
                        <main>
                            ...
                            {/* </Link> */}
                            {/* <button onClick={this.checkToken}>지워줘</button>
                            <h3>{token ? "마이페이지" : "로그인/회원가입"}</h3> */}
                        </main>
